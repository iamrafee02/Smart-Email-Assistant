...
function App() {
  ...
  const handleSubmit = async () => {
    ...
    setError('');
    try {
      const respnse = await axios.post("http://localhost:8086/api/email/generate", {
        emailContent,
        tone
      });
      setGeneratedReply(typeof respnse.data === 'string' ? respnse.data : JSON.stringify(respnse.data));
    } catch (error) {
